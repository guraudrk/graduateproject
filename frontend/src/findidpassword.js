    ...
    <div className="container">
      <div className="left-section">
        <div className="logo-container">
          {/* div로 감싼 다음, 그 안에 img,span을 넣는다.*/}
          {/* a로 감싼 다음, img를 설정하면 img를 클릭 시 다른 사이트로 이동할 수 있게 해준다.*/}
          <a href="/main">
            <img
              className="findidpasswordlogo"
              src="https://aikingsejong.s3.ap-northeast-2.amazonaws.com/aikingsejonglogo.png"
              alt="로고"
            />
          </a>
        </div>
        ...
        <form>
          ...
          <span className="findidpasswordtext1">
            아이디/비밀번호를 잊어버리셨다면, 이메일/아이디를 입력해서 아이디/비밀번호를 다시 찾으세요.
          </span>
          <input
            className="typeemail"
            type="text"
          ...
          />
          <button className="findemail" type="submit" onClick={handleIdSubmit}>
            아이디 찾기
          </button>
          ...
          />
          <button className="findidpassword" type="submit" onClick={handlePasswordSubmit}>
            비밀번호 찾기
          </button>
          <span className={passwordMessage === "비밀번호 값을 찾을 수 없습니다." ? "red-text" : "green-text"}>{passwordMessage}</span>
          <a href="/main" className="gotomain">
            메인 페이지로 돌아가시려면 여기를 클릭해주세요.
          </a>
        </form>
      </div>
